function addCollumn (weight){
    ...
    try{document.getElementsByClassName('basic')[0].childNodes[1].append(createdType);}
    catch(error){
    }
}
...
function grClass(action){
    ...
    classes[1] == 'basic' ? (graph.classList.remove(classes[1]), graph.classList.add(action)) : (graph.classList.remove(classes[1]), graph.classList.add('basic'));
